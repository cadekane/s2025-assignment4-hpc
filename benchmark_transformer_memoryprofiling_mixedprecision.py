...
def run_step(model, inputs, optimizer, enable_backward, device, autocast_ctx, scaler=None):
...
def benchmark(model_size="small", batch_size=8, seq_len=512, steps=5, warmup=1, backward=False, 
             profile_mode=False, mixed_precision=False):
    ...
    precision_str = "mixed precision (FP16)" if mixed_precision else "full precision (FP32)"
    print(f"Running benchmark with {model_size} model using {precision_str}")
    print(f"Batch size: {batch_size}, Sequence length: {seq_len}")
    print(f"Profile mode: {'enabled' if profile_mode else 'disabled'}")
    print(f"Backward pass: {'enabled' if backward else 'disabled'}")
        ...
        try:
            ...
            with profile(
                ...
                for step in range(steps):
                    ...
                    # Run the model step
                    run_step(model, dummy_input, optimizer, backward, device, autocast(), scaler)
            ...
            try:
                ...
                torch.cuda.memory._dump_snapshot("mixedprecision_memory_snapshot_backward.pickle")
    ...
    # Timed steps
        ...
        precision = "mixed precision" if mixed_precision else "full precision"
        print(f"[{model_size}] {kind} pass ({precision}) — Avg: {avg_time:.4f}s, Std Dev: {std_dev:.4f}s")
